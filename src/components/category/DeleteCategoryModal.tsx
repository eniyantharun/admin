...
export function DeleteCategoryModal({
  ...
  const handleDelete = async () => {
    if (hasChildren && !redirectTo) {
      toast.error('Please select a category to redirect to');
      return;
    }

    setLoading(true);
    try {
      await CategoryCRUDService.deleteCategory({
        categoryId,
        redirectToCategoryId: redirectTo,
      });
          ...
          {hasChildren ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-sm text-yellow-800">
                This category has subcategories. You must select a category to redirect to before deleting.
              </p>
            </div>
          ) : (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3">
              <p className="text-sm text-red-800">
                This action cannot be undone. All slug aliases will be transferred if you select a redirect category.
              </p>
            </div>
          )}
