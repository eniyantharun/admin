...
import type { ThemeDetailedStats } from '@/types/api/theme';

interface ProductStatsDisplayProps {
  themeId: number;
}

export function ProductStatsDisplay({ themeId }: ProductStatsDisplayProps) {
  const [stats, setStats] = useState<ThemeDetailedStats | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        const data = await ThemeStatsService.getThemeDetailedStats(themeId);
        setStats(data);
      } catch (error) {
        console.error('Failed to load theme stats:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="border-b border-gray-200 pb-3">
        <p className="text-gray-700">
          <span className="font-medium">Subthemes Count:</span>{' '}
      ...
      {/* Products in theme and subthemes */}
      <div className="border-b border-gray-200 pb-3">
        <p className="text-gray-700 mb-1">
          <span className="font-medium">Products in theme and subthemes</span>
        </p>
        <p className="text-gray-600 ml-4">
          E / D / B:{' '}
